...
import { useCreateAdRequestMutation } from "@/store/services/adsApiSlice";
import {
  createAdRequest as CreateAdRequestType,
  PropertyUserRole,
} from "@/types/ads";
...
const PlaceAnAdModal: React.FC<PlaceAnAdModalProps> = ({ isOpen, onClose }) => {
  ...
  const categories = [
    {
      ...
      image: "/images/propertysale.png",
    },
    {
      id: "rent",
      name: "Property For Rent",
      image: "/images/propertyrent.png",
    },
    { id: "jobs", name: "Jobs", image: "/images/jobs.png" },
  ];

  const [createAdRequest, { isLoading: isSubmitting }] =
  ...
  const handleComplete = async () => {
    ...
    // Map categories to AdType
    let adType: "PROPERTY_SALE" | "PROPERTY_RENT" | "JOB";
    if (selectedCategory === "sale") adType = "PROPERTY_SALE";
    else if (selectedCategory === "rent") adType = "PROPERTY_RENT";
    else adType = "JOB";

    // Map role to PropertyUserRole
    const userRole = role === "Landlord" ? "LANDLORD" : "AGENT";

    // Map Job Sub Category to Category string
    const categoryName = jobSubCategories.find(
      (c) => c.id === selectedJobSubCategory,
    )?.name;

    try {
      const payload: CreateAdRequestType = {
        adType,
        title,
        contact: {
      ...
      if (adType === "JOB") {
        ...
        payload.userRole = role === "Landlord" ? "LANDLORD" : "AGENT";
      }

      await createAdRequest(payload).unwrap();
      setStep(5);
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-110 flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div
        className={`relative w-full ${step === 5 ? "max-w-2xl" : "max-w-2xl"} bg-[#F8F9FA] rounded-[40px] shadow-2xl p-8 md:p-10 animate-in zoom-in-95 fade-in duration-300`}
      >
        {/* Header Actions */}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col items-center text-center max-w-xl mx-auto">
            ...
            <div className="w-full space-y-4">
              ...
              <input
                ...
                className="w-full px-4 py-3 text-black bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/10 focus:border-[#2D5BFF] outline-none text-sm font-medium text-left"
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 text-black bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/10 focus:border-[#2D5BFF] outline-none text-sm font-medium"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 text-black bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/10 focus:border-[#2D5BFF] outline-none text-sm font-medium"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-3">
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-3 text-black bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/10 focus:border-[#2D5BFF] outline-none text-sm font-medium"
                  />
                </div>
              </div>
