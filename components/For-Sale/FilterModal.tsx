...
const FilterModal: React.FC<FilterModalProps> = ({ isOpen, onClose }) => {
    ...
    const [selectedAmenities, setSelectedAmenities] = useState<string[]>(["Free Wifi"]);
            ...
            <div className="relative w-full max-w-xl bg-white md:rounded-[40px] shadow-2xl h-[92vh] md:h-auto md:max-h-[90vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-500 rounded-t-[40px] mt-auto md:mt-0">
                ...
                <div className="flex-1 overflow-y-auto px-6 md:px-8 py-6 space-y-8 md:space-y-10 custom-scrollbar">
                    ...
                    <section className="space-y-4 md:space-y-6">
                        <h3 className="text-lg md:text-xl font-bold text-gray-900">Property Type</h3>
                        ...
                        <div className="grid grid-cols-3 p-1.5 bg-gray-50 border border-gray-100 rounded-full w-full max-w-lg">
                            {["All Type", "Residential", "Commercial"].map((cat) => (
                                <button
                                    ...
                                    className={`py-2 md:py-2.5 px-3 md:px-4 rounded-full font-bold text-xs md:text-sm transition-all duration-300 ${propertyCategory === cat
                        ...
                        <div className="flex flex-wrap gap-3">
                            {[
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-full border transition-all duration-300 font-bold text-xs md:text-sm ${propertyType.includes(type.name)
                    ...
                    {/* Rooms and Beds */}
                    <section className="space-y-4 md:space-y-6">
                        <h3 className="text-lg md:text-xl font-bold text-gray-900">Rooms and Beds</h3>
                        <div className="space-y-2">
                            <Counter label="Rooms" value={rooms} onChange={setRooms} />
                            <Counter label="Beds" value={beds} onChange={setBeds} />
                            <Counter label="Bathroom" value={bathrooms} onChange={setBathrooms} />
                        </div>
                    </section>

                    {/* Area and Size */}
                    <section className="space-y-6 md:space-y-8">
                        <h3 className="text-lg md:text-xl font-bold text-gray-900">Area and Size</h3>
                        ...
                        <div className="px-2 pt-10 pb-4 pr-10 md:pr-12">
                            <div
                                ...
                                onClick={(e) => {
                                    ...
                                    if (distMin < distMax) {
                                        setAreaRange(prev => ({ ...prev, min: Math.min(Math.round(val), prev.max - MIN_DIFF) }));
                                    } else {
                                        setAreaRange(prev => ({ ...prev, max: Math.max(Math.round(val), prev.min + MIN_DIFF) }));
                                    }
                                }}
                            >
                                ...
                                {/* Left Handle */}
                                <div
                                    className={`absolute top-1/2 -translate-y-1/2 w-6 h-6 bg-[#FF5A3D] border-[4px] border-white rounded-full shadow-lg cursor-grab active:cursor-grabbing transform -translate-x-1/2 z-20 transition-transform ${dragging === 'min' ? 'scale-125' : 'hover:scale-110'}`}
                                ...
                                {/* Right Handle */}
                                <div
                                    className={`absolute top-1/2 -translate-y-1/2 w-6 h-6 bg-[#FF5A3D] border-[4px] border-white rounded-full shadow-lg cursor-grab active:cursor-grabbing transform -translate-x-1/2 z-20 transition-transform ${dragging === 'max' ? 'scale-125' : 'hover:scale-110'}`}
                                ...
                                {/* Label */}
                                <span className="absolute -right-12 top-1/2 -translate-y-1/2 text-gray-900 font-bold text-sm">sqft</span>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="space-y-4 md:space-y-6 pb-4">
                        <h3 className="text-lg md:text-xl font-bold text-gray-900">Amenities</h3>
                        <div className="flex flex-wrap gap-3">
                            {[
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-4 md:px-5 py-2 md:py-2.5 rounded-full border transition-all duration-300 font-bold text-xs md:text-sm ${selectedAmenities.includes(amenity.name)
                                        ? "bg-white border-gray-900 text-gray-900 shadow-md ring-1 ring-gray-900"
